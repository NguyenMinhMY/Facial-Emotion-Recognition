import numpy as np
from keras.layers import Flatten,Dense,Activation,Input,Add,Conv2D,ZeroPadding2D, \
    MaxPooling2D,AveragePooling2D,ZeroPadding2D,BatchNormalization,Dropout
...
def ResNet50(input_shape=(48,48,1)):

    X_input = Input(input_shape)
    X = ZeroPadding2D(padding=1)(X_input)

    X = Conv2D(32, (5, 5), strides=(2, 2),padding='valid', name='conv1')(X)
    ...
    #X = Dense(512, activation='relu',name='fc2',kernel_initializer='he_normal' )(X)
    X = Dense(7, activation='softmax',name='fc1', kernel_initializer='he_normal')(X)
